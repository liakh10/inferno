...
function onAccounts(a) { W.address = a && a[0] ? a[0] : null; if (!W.address) { try { localStorage.removeItem('inferno:wallet'); } catch {} } emit(); }
...
export async function connect(id) {
  ...
  try { localStorage.setItem('inferno:wallet', d ? d.info.rdns : 'injected'); } catch {}
  emit();
  return state();
}

export async function restore() {
  let rdns = null; try { rdns = localStorage.getItem('inferno:wallet'); } catch {}
...
export function disconnect() {
  W.address = null; try { localStorage.removeItem('inferno:wallet'); } catch {}
  emit();
}
